...
bool solve_knight_problem(pp_chessboard chessBoard, int* position_x, int* position_y, int* size_x, int* size_y, int current) {
	...
	int  valids[8][3];
	if ((current) == (*size_x) * (*size_y)) return true; // recursion end condition
	for (int  i = 0; i < 8; i++) {
	...
	for (int i = 0; i < 8; i++)
		if (valid(&(valids[i][1]), &(valids[i][2]), chessBoard, size_x, size_y) && solve_knight_problem(chessBoard, &(valids[i][1]), &(valids[i][2]), size_x, size_y, ++current))
		return true;
	printf("\n\n");
